...
var worldhouseSearch = {
    searchSubmit: function () {
        ...
        $subBtn.click(function (ev) {
            ...
            var startDate = $("#internationalStartDate").length > 0 ? parseDate($("#internationalStartDate").val()) : parseDate($("#startDate").val());
            var endDate = $("#internationalEndDate").length > 0 ? parseDate($("#internationalEndDate").val()) : parseDate($("#endDate").val());

            if (!startDate || !endDate || compareDate(startDate, endDate)) {

                if ($("#internationalStartDate").length > 0) {
                    var api = $("#internationalStartDate").data("dateinput");
            ...
            self.setCookie();
            self.doRedirect(currentDestination, isCurrentDuanzu, landmarkPinyin, conditionArray, query_param, self.lm);
        });
    },
    ...
    doRedirect: function (currentDestination, isCurrentDuanzu, landmarkPinyin, conditionArray, query_param, lm) {
        var url = worldUrl;
        ...
        var method = $.initSearch.conf.ui.$submitForm.attr("method")
        ...
        $adress.focus(function () {
            adressFocusTimeout = setTimeout(function () {
                ...
                if ($addressDrop.length === 0) {
                    ...
                    self.resetPosition($adress, $("#worldaddressDrop"), [0, 0]);
    ...
    destSearch: function (kwd) {
        var url = worldUrl + "/KeyWordSearch/?regionid=0&keywordtype=r&keyword=" + encodeURIComponent(kwd) + "&agent=2";
        var self = this;
    ...
    kwdSearch: function (kwd) {
        var self = this;
        var url = worldUrl + "/KeyWordSearch/?regionid=" + self.searchData.DestinationId + "&keywordtype=lm_u&keyword=" + encodeURIComponent(kwd) + "&agent=2";
    ...
    resShow: function (resData, $input) {
        ...
        if ($kwdList.length === 0) {
            ...
            $kwdList.css({
                position: "absolute",
                left: $input.offset().left + self.kwdOffset[0] - 51,
                top: $input.offset().top + $input.outerHeight() + self.kwdOffset[1]
            });
        ...
        if (typeof resData === "string") {
            ...
            $.each(displaywithGroupData, function (key, valObj) {
                ...
                $.each(valObj, function (index, item) {
                    ...
                    liString += '</span>';
                    if (index == 0) {
                        liString += '<span class="keywordItem">' + item.ConditiontTypeName + '<i class="' + item.IconClass + '"></i></span>';
                    }
                    liString += '</a>';
    ...
    resetPosition: function (elem, root, offset) {
        var bodyWidth = $(document.body).outerWidth(true);
        var eloffset = elem.offset(), elemW = elem.innerWidth(), elemH = elem.innerHeight();
        var posLeft = eloffset.left + root.outerWidth(true) + this.destOffset[0];
        var posTop = eloffset.top + elem.outerHeight(true) + this.destOffset[1];
        ...
        if (offset) {
            root.css({
                position: "absolute",
                left: posLeft - root.outerWidth(true) + offset[0],
                top: posTop + offset[1],
                zIndex: 9999
            });
        } else {
    ...
    dateInputInit: function ($startDate, $endDate) {
        ...
        $startDate.dateinput({
            ...
            value: new Date(+mindate + 24 * 3600000),
            offset: self.dateOffset || [0, 0]
        });

        $endDate.dateinput({
            min: new Date(+mindate + 48 * 3600000),
            max: maxdate,
            value: new Date(+mindate + 48 * 3600000),
            offset: self.dateOffset || [0, 0]
        });
